...
function deleteAllScreenshots(clientId) {
  ...
  try {
    ...
    logWithTime(`[DELETE] Deleted all ${deletedCount} screenshot files for client "${clientId}"`);
    ...
    errorWithTime(`[DELETE] Failed to delete all screenshots for ${clientId}:`, error);
    throw error;
  }
}
